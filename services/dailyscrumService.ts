import {
  ASCII_STARTING_INDEX,
  NUMBER_OF_GROUPS,
} from "../constants/dailyscrum";
...
export const findGroupSnapshotByDatetime = (datetime: string) => {
  const groupSnapshot = db.data?.dailyscrum_group_snapshots.filter(
    (snapshot) => snapshot.datetime === datetime
  )[0]; // 하나만 반환함. 생성이나 업데이트 시에 datetime이 중복되지 않게 구현해두었음.
  if (groupSnapshot) {
...
const getRandomGroups = (members: string[]): Groups => {
  const result: Groups = {};
...
const getSnapshotOverlapIndex = (
  prevGroupSnapshots: GroupSnapshot[],
  datetime: string
) => {
  let overlapIndex = -1;
...
export const createGroupSnapshot = async (
  datetime: string
): Promise<GroupSnapshot | GroupErrorMessage> => {
  if (db.data === null)
    return "DB에 데이터가 없습니다. 개발자에게 어서 빨리 초기 세팅을 하라고 재촉해주세요.";
  const members = getMembers();
  if (members === undefined) return "멤버가 아무도 없습니다.";

  const newGroups = getRandomGroups(members);
  const newGroupSnapshot = create<GroupSnapshot>({ datetime, groups: newGroups });
